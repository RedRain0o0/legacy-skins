...
public class ModrinthOauth {
		...
		Thread thread = new Thread() {
			...
			private void run0() throws Throwable {
				...
				server.createContext("/", exchange -> {
					LOGGER.info("Received at URL: " + exchange.getRequestURI());
					LOGGER.info("Received something? " + new String(exchange.getRequestBody().readAllBytes()));
					String query = exchange.getRequestURI().getQuery();
					LOGGER.info(query);
					if (query.contains("code=") && query.split("=").length == 2) {
						String code = query.split("=")[1];
						LOGGER.info("Modrinth has responded with a code: " + code);
	...
	public static CompletableFuture<ModrinthDataObjects.OauthTokenResponse> getToken(ModrinthDataObjects.OauthTokenPostDto dto) {
		...
		return client.sendAsync(builder().header("Content-Type", "application/x-www-form-urlencoded").header("Authorization", OAUTH_SECRET).POST(HttpRequest.BodyPublishers.ofString(formText, StandardCharsets.UTF_8 /*TODO*/)).uri(URI.create(TOKEN_URL)).build(), HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8)).thenApply(response -> {

...
public record LegacySkinPack(ResourceLocation icon, List<LegacySkin> skins) {
    ...
    public static class Manager implements SimpleResourceReloadListener<List<LegacySkinPack>> {
		@Override
		public CompletableFuture<List<LegacySkinPack>> load(ResourceManager resourceManager, ProfilerFiller profiler, Executor executor) {
		...
		@Override
		public CompletableFuture<Void> apply(List<LegacySkinPack> data, ResourceManager manager, ProfilerFiller profiler, Executor executor) {
			list.clear();
			list.addAll(data);

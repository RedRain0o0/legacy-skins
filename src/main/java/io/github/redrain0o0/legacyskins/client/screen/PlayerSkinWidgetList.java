...
public class PlayerSkinWidgetList {
	...
	public void sortForIndex(int index) {
		...
		// 1 -> [5, 6, 2, 3, 4]
		// etc
		elementN1 = get(index - 4);
		elementN1.interactable = false;
		elementN1.invisible();
		elementN1.beginInterpolation(0, FACING_FROM_LEFT, CENTER_X - OFFSET * 4 + 80, CENTER_Y + VERTICAL_OFFSET + 10, 0.4f);
		...
		element7.invisible();
		element7.beginInterpolation(0, FACING_FROM_RIGHT, CENTER_X + OFFSET * 4, CENTER_Y + VERTICAL_OFFSET * 4 + 20, 0.4f);
	}
